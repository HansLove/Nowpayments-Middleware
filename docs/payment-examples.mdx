...
For scenarios where you want to accept a fixed fiat amount in a specific cryptocurrency without pre-creating an invoice:

```typescript
// Accept exactly $100 USD paid in USDT on Polygon
app.post('/checkout',
  ...
  (req, res) => {
    ...
    // Display in your UI: "Send X USDT to address Y"
    res.json({
      success: true,
      payment,
    });
...
**Key differences from `createPaymentByInvoice`:**
- No need to create an invoice beforehand
- Internally creates invoice + payment in one step
- Same response format as other payment middlewares
